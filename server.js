...
║ 💳 Debt: ${u.debt || 0} XS
║ 💼 Government Job: ${u.job ? u.job.title : 'None'}
║ 🏢 Company: ${u.company || 'None'}
║ ⚠️ Warns: ${u.warns || 0}/${CONFIG.WARN_LIMIT}
...
║ 🎓 Class: ${getUserClass(sender)}
║ ⭐ Level: ${u.level}
║ 💼 Gov Job: ${u.job ? u.job.title : 'None'}
║ 🏦 Bank Job: ${u.bankJob ? u.bankJob.title : 'None'}
║ 🏢 Company: ${u.company || 'None'}
║ 💰 Wallet: ${u.xenoShards} XS
║ 💳 Debt: ${u.debt || 0} XS
...
║ 🛡️ 𝗔𝗱𝗺𝗶𝗻𝘀: ${meta.participants.filter(p => p.admin).length}
║
║ 💬 𝗖𝗵𝗮𝘁: ${settings.chat ? 'true' : 'false'}
║ 🔗 𝗚𝗿𝗼𝘂𝗽 𝗟𝗶𝗻𝗸: available
║ 🕵️‍♂️ 𝗔𝗻𝘁𝗶-𝗦𝗽𝗮𝗺: ${settings.antiSpam ? 'on' : 'off'}
║ 🚫 𝗕𝗹𝗮𝗰𝗸𝗹𝗶𝘀𝘁: ${BLACKLIST.length ? BLACKLIST.length + ' users' : 'none'}
║
║ 🎴 𝗖𝗮𝗿𝗱𝘀: removed
║ 🎮 𝗚𝗮𝗺𝗲𝘀: ${settings.games === false ? 'off' : 'on'}
║ 🎰 𝗚𝗮𝗺𝗯𝗹𝗶𝗻𝗴: removed
╚═════════════╝`;
}
...
3. No scams.
4. Pay your debts.
5. Respect group administration.
6. Do not abuse Hodekai.
7. Government jobs require the correct status.
8. Company members cannot hold government jobs.
9. CONCLAVE protects its economy.
10. Higher authority is earned and recorded.`;
}
...
function bankBox(sender) {
  const b = BANKS[clean(sender)];
  return `╔═ ❰ 🏦 𝗖𝗢𝗡𝗖𝗟𝗔𝗩𝗘 𝗕𝗔𝗡𝗞 ❱ ═╗
║ Account: ${b ? b.account : 'Not opened'}
║ Balance: ${b ? b.balance : 0} XS
║
║ ${PREFIX}bank open
║ ${PREFIX}bank bal
║ ${PREFIX}bank dep <amount>
║ ${PREFIX}bank wit <amount>
║ ${PREFIX}bank send <profileId> <amount>
║ ${PREFIX}bank stmt
║ ${PREFIX}bank close
║
║ Loans require:
...
║
║ Loan interest: 0.3% every 2 days
╚════════════════════╝`;
}
...
${PREFIX}company hire <number>
${PREFIX}company fire <number>
${PREFIX}company role <number> <role>
${PREFIX}company pay <number> <amount>
${PREFIX}company tax
${PREFIX}company top
...
╚════════════════════╝`;
  ...
  if (FUN_COMMANDS.includes(c)) {
    if (c === 'joke') return `😈 ${random([
      'Why did the bot cross the server? To avoid your code.',
      'I would tell you a UDP joke, but you might not get it.',
      'I tried to become human. The bugs were unbearable.'
    ])}`;
  ...
  if (['kick', 'mute', 'unmute', 'warn', 'tagall'].includes(c)) {
    if (!groupId) return '❌ Groups only.';
  ...
  if (c === 'logs') {
    ...
    return `╔═ ❰ 🕵️ 𝗖𝗢𝗡𝗖𝗟𝗔𝗩𝗘 𝗟𝗢𝗚𝗦 ❱ ═╗
${LOGS.slice(0, 15).map((x, i) =>
...
╚════════════════════╝`;
  ...
  if (c === 'viewall') {
    if (!isOwner(sender)) return '❌ Owner command.';
    return `📊 USERS\n${Object.values(USERS).slice(0, 30)
      .map(x => `${x.profileId} | ${x.username} | ${x.xenoShards} XS | ${x.status}`)
      .join('\n')}`;
  }
