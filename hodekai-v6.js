...
const MENU_SECTIONS = [
    ...
    { title: "💼 JOBS & COMPANY", items: [
        ...
        [`${PREFIX}company hire`, "Hire (reply/mention)"], [`${PREFIX}company fire`, "Fire (reply/mention)"], [`${PREFIX}company pay`, "Pay employee"],
...
async function handleCommand(cmd, args, sender, groupId, sock, msg, contextInfo, isDM, isWaAdminFlag, groupMeta) {
    ...
    if (c === 'company') {
        ...
        if (sub === 'fire') {
            ...
            c2.employees = c2.employees.filter(e => e !== target); if (USERS[target]) USERS[target].company = null; saveData();
            return `🚪 ${target} fired.`;
        }
        if (sub === 'pay') {
            const c2 = COMPANIES[user.company]; if (!c2 || c2.owner !== sender) return "❌ Only CEO.";
    ...
    if (['kick', 'mute', 'unmute', 'warn', 'close', 'open', 'delete', 'tagall', 'promote', 'demote', 'active', 'inactive', 'gclink'].includes(c)) {
        ...
        const botAdmin = await isBotAdmin(sock, groupId, groupMeta);
        const target = getTarget(args, 0, msg);
        if (c === 'kick') { if (!botAdmin) return "❌ Bot needs to be admin."; if (!target) return `❌ Reply or ${PREFIX}kick @user`; if (isProtected(target)) return "❌ Protected."; try { await sock.groupParticipantsUpdate(groupId, [target + "@s.whatsapp.net"], "remove"); addLog('KICK', sender, target); return `⚠️ ${target} kicked.`; } catch (e) { return `❌ ${e.message}`; } }
        ...
        if (c === 'delete') { if (!botAdmin) return "❌ Bot needs to be admin."; const ctx = msg.message?.extendedTextMessage?.contextInfo; if (!ctx?.stanzaId) return `❌ Reply to a message with ${PREFIX}delete`; try { await sock.sendMessage(groupId, { delete: { remoteJid: groupId, fromMe: false, id: ctx.stanzaId, participant: ctx.participant } }); return null; } catch (e) { return `❌ ${e.message}`; } }
        if (c === 'tagall') {
        ...
        if (c === 'gclink') { if (!botAdmin) return "❌ Bot needs to be admin."; try { const code = await sock.groupInviteCode(groupId); return `🔗 https://chat.whatsapp.com/${code}`; } catch (e) { return `❌ ${e.message}`; } }
